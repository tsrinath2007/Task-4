    ...
    def _mock_response(self, prompt_type: str, ctx: InvestigationContext) -> Dict[str, Any]:
        ...
        if prompt_type == "adjudicator":
            ...
            if verdict == "fraud":
                ...
                prob = round(min(0.965, max(0.840, base + card_boost + amt_boost + risk_boost + micro)), 3)
                ...
                prob = round(0.042 + amt_factor + card_factor + micro, 3)
            else:  # uncertain
                prob = round(0.515 + (raw_risk - 0.5) * 0.12 + ((cid_num * 7) % 11) * 0.002, 3)
